...
#define CPPLINT_FORALL_ONE_OR_TWO_CHAR_TOKENS2(apply)               \
  apply('+', TK_PLUS, '+', TK_INCREMENT, '=', TK_PLUS_ASSIGN)       \
  apply('&', TK_AMPERSAND, '&', TK_LOGICAL_AND, '=', TK_AND_ASSIGN) \
  apply('|', TK_BINARY_OR, '|', TK_LOGICAL_OR, '=', TK_OR_ASSIGN)
...
#define CPPLINT_FORALL_ONE_TO_THREE_CHAR_TOKENS(apply)                           \
  apply('<', TK_LESS, '=', TK_LESS_EQUAL, '<', TK_LSHIFT, '=', TK_LSHIFT_ASSIGN) \
  apply('>', TK_GREATER, '=', TK_GREATER_EQUAL, '>', TK_RSHIFT, '=', TK_RSHIFT_ASSIGN)
